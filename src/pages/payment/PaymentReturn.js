...
const PaymentReturn = () => {
  ...
  const handleCreateOrder = async (data) => {
    const productItemIds = cartItems?.map(cartItem => cartItem?.id) ?? [];
    try {
      const parsedData = {
      ...
      await dispatch(orderApi.endpoints.createOrder.initiate(parsedData)).unwrap()
      return true;
    } catch (error) {
      console.log(error)
    }
  ...
  useEffect(() => {
    const execPayment = async () => {
      ...
      const orderInfo = paymentResult["vnp_OrderInfo"] ? JSON.parse(paymentResult["vnp_OrderInfo"]) : {}
      ...
      if (secureHash === signed) {
        ...
        if (responseCode === "00") {
          ...
          if (isCreatedOrder) setPaymentStatus(PAYMENT_STATUS.success);
  ...
  const renderContent = () => {
    switch (paymentStatus) {
      case PAYMENT_STATUS.idle:
        return <Stack alignItems='center' spacing={1.5}>
          <CircularProgress />
      ...
      case PAYMENT_STATUS.wrongData:
        return <ErrorCard title='Thông tin không hợp lệ'
          description="Hàm băm an toàn không hợp lệ. Dữ liệu thanh toán có thể bị giả mạo."
        />
      case PAYMENT_STATUS.failed:
        return <ErrorCard title='Thanh toán thất bại'
  ...
  return (
    <Box textAlign='center' my={3}>
      <Typography variant="h4" py={1.5}>Thông tin thanh toán</Typography>
      {renderContent()}
    </Box>
  );
};
